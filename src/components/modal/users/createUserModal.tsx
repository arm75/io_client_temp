...
function CreateUserModal(props:any) {
    ...
    content = <> 
        {/* <Modal  isOpen={isOpen} onClose={closeModal}> */}
            <form className="form" onSubmit={form.handleSubmit(submitCreateUserForm)}>
                
                <label className="form__label" htmlFor="username"> 
                    Username: <span className="nowrap">[3-20 chars]</span>
                </label>          

                <input {...form.register("username", { pattern: /^[A-z][A-z0-9]{2,19}$/, required: true, minLength: 3, maxLength: 20 })} aria-invalid={form.formState.errors?.username ? "true" : "false"} id="username" name="username" type="text" autoComplete="off"/>
                {form.formState.errors?.username?.type === 'pattern' && <p role="alert">Username can be up to 20 characters, and must start with a letter.</p>}
                {form.formState.errors?.username?.type === 'required' && <p role="alert">Username is required</p>}
                {form.formState.errors?.username?.type === 'minLength' && <p role="alert">Username must be at least 3 characters.</p>}
                {form.formState.errors?.username?.type === 'maxLength' && <p role="alert">Username must be less than 20 characters.</p>}
                    
                <label className="form__label" htmlFor="password"> 
                    Password: <span className="nowrap">[4-12 chars incl. !@#$%]</span>
                </label> 

                <input {...form.register("password", { required: true, minLength: 4, maxLength: 12 })} id="password" name="password" type="password"/>

                <label className="form__label" htmlFor="roles"> 
                    Roles:
                </label>

                <select {...form.register("roles", { required: true })} id="roles" name="roles" multiple={true}>                                             
